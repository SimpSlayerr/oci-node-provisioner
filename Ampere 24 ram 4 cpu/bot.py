...
def main():
    ...
    calc_fingerprint, _ = get_key_details(private_key)
    fingerprint = calc_fingerprint if calc_fingerprint else secret_fingerprint

    print("=" * 60)
    print(f"AUTHENTICATING WITH:")
    print(f"Tenancy OCID: {tenancy_id[:25]}... (length: {len(tenancy_id)})")
    print(f"User OCID:    {user_id[:25]}... (length: {len(user_id)})")
    ...
    try:
        ...
        print(f"✅ SUCCESS: Authenticated! Found {len(ads)} Availability Domains in {region}.")
    except oci.exceptions.ServiceError as e:
        print(f"\n❌ OCI Authentication Error:")
        print(f"Status:  {e.status}")
        print(f"Code:    {e.code}")
        print(f"Message: {e.message}")
        print(f"Details: {e.request_endpoint}")
        sys.exit(1)
    except Exception as e:
    ...
    print("Testing availability domains for ARM host capacity...")
        ...
        for fd in fault_domains:
            ...
            try:
                ...
                if response.data.lifecycle_state in ["PROVISIONING", "RUNNING"]:
                    print(f"🎉 INSTANCE CREATED: {response.data.id}")
                    sys.exit(0)
            except oci.exceptions.ServiceError as e:
